...
class ItemBuilderEngine:
    ...
    def __init__(self):
        self.recorded_row = pd.Series(data=[])
        self.recorded_data = pd.DataFrame(data={})

    def get_recorded_testcase(self) -> str | None:
    ...
    def init_first_row(self, instance):
        self.recorded_data = pd.read_csv(instance.get_replay_file(), index_col="index")
        self.recorded_data = self.recorded_data.query(f"test_case == \"{instance.get_current_test_case()}\"")
        if(0 < len(self.recorded_data)):
            self.recorded_row = next(self.recorded_data.iterrows())[1]
        else:
            self.recorded_row = pd.Series(data={ "timestamp":"", "item":"unknown", "value": "", "test_suite": f"{instance.get_current_test_suite()}", "test_case": f"{instance.get_current_test_case()}", "keyword": ""}, index=["timestamp", "item", "value", "test_suite", "test_case", "keyword"])
        ...
        if ( instance.MODE_DEF == instance.get_mode() ):
            ...
            if ( None == instance.Get_Context() ):
                ...
                if ( "de_DE" == localization ):
                    # Rule R01 ---->
                    # Trace ---->
                    instance.add_rbt_log_message("[SynData] Decision table ItemBuilderEngine (20260321.133620), rule 1 of 9 has been executed", "DEBUG")
                    # Trace <----
                    ...
                    # Action A06/01: Return data from ItemBuilderEngine... / X / Aktion ausführen
                    return retval
                    # Rule R01 <----
                else:
                    # Rule R02 ---->
                    # Trace ---->
                    instance.add_rbt_log_message("[SynData] Decision table ItemBuilderEngine (20260321.133620), rule 2 of 9 has been executed", "DEBUG")
                    # Trace <----
                ...
                if ( None == instance.get_item(item) ):
                    ...
                    if ( "de_DE" == localization ):
                        # Rule R03 ---->
                        # Trace ---->
                        instance.add_rbt_log_message("[SynData] Decision table ItemBuilderEngine (20260321.133620), rule 3 of 9 has been executed", "DEBUG")
                        # Trace <----
                        # Action A01/02: Call ItemBuilderEngine... / Germany / This item builder is responsible for german test data.
                        retval = ItemBuilderEngineGermany.execute(instance, localization, keyword, item, item_data)
                        ...
                        # Rule R04 ---->
                        # Trace ---->
                        instance.add_rbt_log_message("[SynData] Decision table ItemBuilderEngine (20260321.133620), rule 4 of 9 has been executed", "DEBUG")
                        # Trace <----
                        # Action A01/01: Call ItemBuilderEngine... / Faker / If there is no reliable implementation for the defined localization, Faker is used as a substitute.
                        retval = ItemBuilderEngineFaker.execute(instance, localization, keyword, item, item_data)
                        # Action A03/02: Call for logging / stored / The value should be read from the data structure for logging.
                    ...
                    # Rule R05 ---->
                    # Trace ---->
                    instance.add_rbt_log_message("[SynData] Decision table ItemBuilderEngine (20260321.133620), rule 5 of 9 has been executed", "DEBUG")
                    # Trace <----
                    # Action A03/02: Call for logging / stored / The value should be read from the data structure for logging.
                    value = instance.get_item(item)
                    instance.add_log_entry(keyword, item, value)
                    # Action A05/01: Return data stored in the current context / X / Aktion ausführen
            ...
            if ( instance.get_current_test_case() == self.get_recorded_testcase()):
                ...
                if (item == f"{self.recorded_row["item"]}"):
                    # Rule R06 ---->
                    # Trace ---->
                    instance.add_rbt_log_message("[SynData] Decision table ItemBuilderEngine (20260321.133620), rule 6 of 9 has been executed", "DEBUG")
                    # Trace <----
                    ...
                    if ( "de_DE" == localization ):
                        # Rule R07 ---->
                        # Trace ---->
                        instance.add_rbt_log_message("[SynData] Decision table ItemBuilderEngine (20260321.133620), rule 7 of 9 has been executed", "DEBUG")
                        # Trace <----
                        # Action A01/02: Call ItemBuilderEngine... / Germany / This item builder is responsible for german test data.
                        retval = ItemBuilderEngineGermany.execute(instance, localization, keyword, item, item_data)
                        # Action A03/01: Call for logging / local / The locally generated value should be transferred to the logging
                        ...
                        instance.add_rbt_log_message(f"[SynData.NOREP] According to the data record, the keyword {self.recorded_row["keyword"]} was expected to be called. The return value therefore does not correspond to the recorded value, but to a generated value.", "WARN")
                        # Action A06/01: Return data from ItemBuilderEngine... / X / Aktion ausführen
                        return retval
                        # Rule R07 <----
                    else:
                        # Rule R08 ---->
                        # Trace ---->
                        instance.add_rbt_log_message("[SynData] Decision table ItemBuilderEngine (20260321.133620), rule 8 of 9 has been executed", "DEBUG")
                        # Trace <----
                        # Action A01/01: Call ItemBuilderEngine... / Faker / If there is no reliable implementation for the defined localization, Faker is used as a substitute.
                        retval = ItemBuilderEngineFaker.execute(instance, localization, keyword, item, item_data)
                        # Action A03/01: Call for logging / local / The locally generated value should be transferred to the logging
                ...
                # Rule R09 ---->
                # Trace ---->
                instance.add_rbt_log_message("[SynData] Decision table ItemBuilderEngine (20260321.133620), rule 9 of 9 has been executed", "DEBUG")
                # Trace <----
                # Action A02/01: Initialization of recorded data for the current test case / X / Aktion ausführen
                self.init_first_row(instance)
